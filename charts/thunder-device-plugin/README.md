...
- **operator** — a Deployment that reads Thunder inventory and publishes one
  `ResourceSlice` per zone and GPU type.
...
| `DRAConsumableCapacity` feature gate | Alpha; required on API server, scheduler, controller-manager and kubelet |
...
| `operator.validGPUCounts` | list | `["1"]` | GPU counts a single claim may request |
